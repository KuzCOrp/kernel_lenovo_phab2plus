...
static int escore_non_vs_suspend(struct device *dev)
{
	...
	/*Disable Bellamy Mic before entering sleep mode*/
	escore_main_mic_turnon(0);
	msleep(1);
...
static int escore_vs_suspend(struct device *dev)
{
	...
	dev_dbg(dev, "%s()\n", __func__);
	/*Enable Bellamy Mic before entering VQ mode*/
	escore_main_mic_turnon(1);
...
static int escore_system_suspend(struct device *dev)
{
	...
	if (!es_ready_to_suspend(escore)) {
		dev_info(dev, "%s() - Not ready for suspend\n", __func__);
		return 0;
	}
