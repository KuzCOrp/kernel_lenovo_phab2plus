...
static int es705_i2c_write_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
	/*
...
int escore_i2c_write(struct escore_priv *escore, const void *buf, int len)
{
	...
	while (len > 0) {
		...
		err = es705_i2c_write_block((struct i2c_client *)escore_i2c, 0, temp_buf, temp_len);
		if (err) {
			printk("lvlili%s--error count:%d,len:0x%x\n",__FUNCTION__,count,len);
			return err;
		}
...
#endif
	printk("STT: escore-i2c is ready!\n");
	return rc;
}
...
#ifdef CONFIG_OF
static const struct of_device_id audience_i2c_of_match[] = {
	{.compatible = "mediatek,I2C_AUDIENCE"},
	{},
};
#endif
...
struct i2c_driver escore_i2c_driver = {
	.driver = {
		.name = "earSmart-codec",
#ifdef CONFIG_OF
		.of_match_table = audience_i2c_of_match,
#endif
		.owner = THIS_MODULE,
		.pm = &escore_pm_ops,
	},
