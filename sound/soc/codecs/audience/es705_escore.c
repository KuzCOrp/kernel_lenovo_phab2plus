...
#if 1 //added by shentaotao
#include <linux/of_irq.h>
struct device_node *audience_node = NULL;
int audience_irq = 0, audience_mic_vdd_1v8_gpio = 0, audience_26m_clk_gpio = 0;
extern struct regulator *vcn18_main_mic_reg;
#endif
...
static int es705_put_delay_count(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	int rc = 0;
	int value = ucontrol->value.enumerated.item[0];
...
static int escore_sub_mic_turnon(int turnon);
int escore_main_mic_turnon(int turnon);
static int es705_get_sub_mic_power(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
...
static int es705_put_sub_mic_power(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	return escore_sub_mic_turnon(ucontrol->value.integer.value[0]);
}

static int es705_get_main_mic_power(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = 0;
...
static int es705_put_main_mic_power(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	return escore_main_mic_turnon(ucontrol->value.integer.value[0]);
}
...
static struct snd_kcontrol_new es705_digital_ext_snd_controls[] = {
	...
	SOC_ENUM_EXT("Algorithm Processing", es705_algo_processing_enable_enum,
		     es705_get_control_enum, es705_put_control_enum),
	SOC_SINGLE_EXT("Sub Mic Power",
		       SND_SOC_NOPM, 0, 1, 0, es705_get_sub_mic_power,
		       es705_put_sub_mic_power),
	SOC_SINGLE_EXT("Main Mic Power",
		       SND_SOC_NOPM, 0, 1, 0, es705_get_main_mic_power,
		       es705_put_main_mic_power),
	...
	SOC_SINGLE_EXT("Preset1",
		       ES705_PRESET, 0, 65535, 0, es705_get_preset_value,
		       es705_put_preset_value),
	SOC_SINGLE_EXT("Preset2",
		       ES705_PRESET, 0, 65535, 0, es705_get_preset_value,
		       es705_put_preset_value),
	SOC_SINGLE_EXT("Preset3",
		       ES705_PRESET, 0, 65535, 0, es705_get_preset_value,
		       es705_put_preset_value),
	SOC_SINGLE_EXT("Preset4",
		       ES705_PRESET, 0, 65535, 0, es705_get_preset_value,
		       es705_put_preset_value),
	...
	SOC_SINGLE_EXT("ESCORE Delay",
			SND_SOC_NOPM, 0, 65535, 0, NULL,
			es705_put_delay_count),
};
...
repeat:
	...
	if (escore_priv.pdata->esxxx_clk_cb) {
		...
		mdelay(ES_PM_CLOCK_STABILIZATION);
	}
...
int audience_es814_26m_clk_turnon(int audience_26m_clock)
{
#if 0 //modified by shentaotao 2016.06.09
	if (audience_26m_clk_gpio >= 0)
	{
		if (audience_26m_clock)
		{
			mt_set_gpio_mode(audience_26m_clk_gpio, 1);//GPIO3 set mode1
		}
		else
		{
			mt_set_gpio_mode(audience_26m_clk_gpio, 0);//GPIO3 set mode0
		}
...
static int escore_sub_mic_turnon(int turnon)
{
	if (audience_mic_vdd_1v8_gpio >= 0)
	{
		if (turnon)
		{
			gpio_set_value(audience_mic_vdd_1v8_gpio, 1);
		}
		else
		{
			gpio_set_value(audience_mic_vdd_1v8_gpio, 0);
		}
...
int escore_main_mic_turnon(int turnon)
{
	...
	if (vcn18_main_mic_reg)
	{
		ret = regulator_set_voltage(vcn18_main_mic_reg, 1800000, 1800000);
		if (ret)
			printk("regulator_set_voltage(vcn18_main_mic_reg) failed!\n");

		if (flag) {
			ret = regulator_enable(vcn18_main_mic_reg);
			if (ret)
				printk("regulator_enable(vcn18_main_mic_reg) failed!\n");
		} else {
			ret = regulator_disable(vcn18_main_mic_reg);
			if (ret)
				printk("regulator_disable(vcn18_main_mic_reg) failed!\n");
		}
	}
	return 0;
}

/* Interface for app layer to turn on main mic power */
struct kobject *audience_voice_wake_kobj = NULL;
static int voice_wake_enable_flag = 0;
static ssize_t voice_wake_enable_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	ssize_t len;

	len = sprintf(buf, "voice_wake_enable_flag = %d\n", voice_wake_enable_flag);
	return len;
}

static ssize_t voice_wake_enable_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	int flag = 0;

	if (!kstrtoint(buf, 10, &flag)) {
		voice_wake_enable_flag = !!flag;
		escore_main_mic_turnon(voice_wake_enable_flag);
	}

	return count;
}
static DEVICE_ATTR(voice_wake_enable, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, voice_wake_enable_show, voice_wake_enable_store);
/* Interface for app layer to turn on main mic power */
...
static struct esxxx_platform_data *es705_populate_dt_pdata(struct device *dev)
{
	...
	if (!pdata) {
		printk("%s: platform data allocation failed\n", __func__);
	}

	audience_node = of_find_compatible_node(NULL, NULL, "mediatek,audience");
	if (audience_node)
	{
		ret = of_property_read_u32(audience_node, "audience_rst_pin", &ints);
		if (ret)
			printk("STT: audience_rst_pin is not OK!\n");

		ret = of_property_read_u32(audience_node, "audience_gpiob_pin", &ints1);
		if (ret)
			printk("STT: audience_gpiob_pin is not OK!\n");

		ret = of_property_read_u32(audience_node, "audience_mic_vdd_1v8_pin", &audience_mic_vdd_1v8_gpio);
		if (ret)
			printk("STT: audience_mic_vdd_1v8 is not OK!\n");

		ret = of_property_read_u32(audience_node, "audience_26m_clk_pin", &audience_26m_clk_gpio);
		if (ret)
			printk("STT: audience_26m_clk_pin is not OK!\n");

		pdata->gpioa_gpio = -1;
		pdata->wakeup_gpio = -1;
		pdata->reset_gpio = ints;
		pdata->gpiob_gpio = ints1;
		audience_irq = irq_of_parse_and_map(audience_node, 0);
		printk("STT: reset_gpio=%d\n", pdata->reset_gpio);
		printk("STT: gpioa_gpio=%d\n", pdata->gpioa_gpio);
		printk("STT: gpiob_gpio=%d\n", pdata->gpiob_gpio);
		printk("STT: wakeup_gpio=%d\n", pdata->wakeup_gpio);
		printk("STT: sub_mic_power=%d\n", audience_mic_vdd_1v8_gpio);
		printk("STT: 26m_clk_gpio=%d\n", audience_26m_clk_gpio);
		printk("STT: audience_irq=%d\n", audience_irq);
	}
	else
	{
	...
	pdata->ext_clk_rate = ES_EXT_CLK_26000KHZ;
	pdata->esxxx_clk_cb = audience_es814_26m_clk_turnon;
...
int es705_core_probe(struct device *dev)
{
	...
	const char *fw_dolby_filename =
		"audience/es814/audience-es814-dolby.bin";
#else
...
*/
	...
	if (pdata->gpiob_gpio != -1) {
		...
		printk("STT: gpiob_gpio=%d, rc=%d\n", pdata->gpiob_gpio, rc);
		gpio_direction_input(pdata->gpiob_gpio);
#else
	...
	if (pdata->gpiob_gpio != -1) {
#if 1 //added by shentaotao
		rc = request_threaded_irq(audience_irq,
					  NULL,
					  es705_irq_work,
		...
		/*
		rc = irq_set_irq_wake(audience_irq, 1);
		if (rc < 0) {
			dev_err(escore_priv.dev,
				"%s(): set event irq wake fail %d\n",
				__func__, rc);
			disable_irq(audience_irq);
			free_irq(audience_irq, &escore_priv);
	...
	if (pdata->reset_gpio != -1) {
		...
		printk("STT: gpio_request(%d)--rc=%d\n", pdata->reset_gpio, rc);
	...
	if (audience_mic_vdd_1v8_gpio >= 0)
	{
		rc = gpio_request(audience_mic_vdd_1v8_gpio, "audience_sub_mic_power");
		printk("STT: gpio_request(%d)--rc=%d\n", audience_mic_vdd_1v8_gpio, rc);
		gpio_set_value(audience_mic_vdd_1v8_gpio, 0);
	}

	escore_main_mic_turnon(0);//turn off main mic power

	audience_voice_wake_kobj = kobject_create_and_add("voice_wake_up", NULL) ;
	if (audience_voice_wake_kobj == NULL)
		printk("%s: subsystem_register failed\n", __func__);

	rc = sysfs_create_file(audience_voice_wake_kobj, &dev_attr_voice_wake_enable.attr);
	if (rc)
