...
extern unsigned char ts3a227e_connector_type;
//Added by shentaotao 2016.06.07
extern int ts3a227e_connector_type_notify;
...
static DEFINE_SPINLOCK(auddrv_i2s0_awb_lock);
...
int  GetAPLLCounter(void)
{
	int Counter = APLLCounter;
	return Counter;
}
#endif
...
bool SetI2SAdcIn(AudioDigtalI2S *DigtalI2S)
{
	...
	//Added by shentaotao 2016.06.07
	printk("STT:ts3a227e_connector_type = %d, ts3a227e_connector_type_notify = %d\n",
		ts3a227e_connector_type, ts3a227e_connector_type_notify);
	if ((ts3a227e_connector_type == 2) || (ts3a227e_connector_type == 3))
		AudioAdcI2SStatus = false;
	else
		AudioAdcI2SStatus = true;

	//++Added by shentaotao 2016.06.07
	if (ts3a227e_connector_type_notify)
		AudioAdcI2SStatus = true;
	//--Added by shentaotao 2016.06.07
	if (false == AudioAdcI2SStatus) {
		...
		/* Modified by Shentaotao Start */
		Audio_I2S_Adc |= (1 << 12);//Enable low jitter
		/* Modified by Shentaotao End */
...
bool Set2ndI2SOutFor3MIC(uint32 SampleRate, bool lowjitter, bool I2SWLen)
{
	uint32 Audio_I2S_Dac = 0;

	pr_warn("Set2ndI2SOutFor3MIC SampleRate %d, lowjitter %d, I2SWLen %d\n", SampleRate, lowjitter,
		I2SWLen);
	CleanPreDistortion();
	SetDLSrc2(SampleRate);
	Audio_I2S_Dac |= (Soc_Aud_LR_SWAP_NO_SWAP << 31);
	...
	Audio_I2S_Dac |= (I2SWLen << 1);
	Audio_I2S_Dac |= (lowjitter << 12);	/* low gitter mode */
	Afe_Set_Reg(AFE_I2S_CON3, Audio_I2S_Dac, MASK_ALL);
...
static unsigned long i2s0_awb_flags;

void Auddrv_I2S0_AWB_Spinlock_lock(void)
{
    spin_lock_irqsave(&auddrv_i2s0_awb_lock, i2s0_awb_flags);
}

void Auddrv_I2S0_AWB_Spinlock_unlock(void)
{
    spin_unlock_irqrestore(&auddrv_i2s0_awb_lock, i2s0_awb_flags);
}
