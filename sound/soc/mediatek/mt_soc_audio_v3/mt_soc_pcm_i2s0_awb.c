...
static AudioDigtalI2S *mAudioDigitalI2S0Awb = NULL;
...
static void Config2ndI2S(struct snd_pcm_substream *substream)
{
    mAudioDigitalI2S0Awb->mLR_SWAP = Soc_Aud_LR_SWAP_NO_SWAP;
    mAudioDigitalI2S0Awb->mINV_LRCK = Soc_Aud_INV_LRCK_NO_INVERSE;
    mAudioDigitalI2S0Awb->mI2S_FMT = Soc_Aud_I2S_FORMAT_I2S;
    mAudioDigitalI2S0Awb->mI2S_WLEN = Soc_Aud_I2S_WLEN_WLEN_16BITS;
    mAudioDigitalI2S0Awb->mI2S_SAMPLERATE = (substream->runtime->rate);
    mAudioDigitalI2S0Awb->mI2S_IN_PAD_SEL = true; /* I2S_IN_FROM_CONNSYS */
    mAudioDigitalI2S0Awb->mI2S_SLAVE = Soc_Aud_I2S_SRC_MASTER_MODE;
}

static void StartAudioI2S0AWBHardware(struct snd_pcm_substream *substream)
{
    uint32 MclkDiv1 = 0, MclkDiv2 = 0; 
    /* Modified by 07315 Start */
    EnableI2SDivPower(AUDIO_APLL12_DIV2, true);
    if(1)
    {
        Afe_Set_Reg(AUDIO_TOP_CON1, 0x2,  0x2);  // I2S0/I2S3 SOFT_Reset Hi
        Afe_Set_Reg(AUDIO_TOP_CON1, (0x1 << 4)|(0x1 << 7),	(0x1 << 4)|(0x1 << 7)); // Sets to 1 to gated I2S0/3 engine clock

        /* start 2nd I2S  in */
        Config2ndI2S(substream);
        Set2ndI2SIn(mAudioDigitalI2S0Awb);

        /* start 2nd I2S out */
        Set2ndI2SOutFor3MIC(substream->runtime->rate, true, Soc_Aud_I2S_WLEN_WLEN_16BITS);
        ...
        Afe_Set_Reg(AUDIO_TOP_CON1, 0x0,  0x2);  // // I2S1/I2S2 SOFT_Reset Lo
						  ...
						  *substream)
{
    ...
    AFE_BLOCK_T *I2S0_AWB_Block = &(I2S0_AWB_Control_context->rBlock);
    PRINTK_AUD_AWB("mtk_i2s0_awb_pcm_pointer Awb_Block->u4WriteIdx;= 0x%x \n", I2S0_AWB_Block->u4WriteIdx);
    //printk("mtk_i2s0_awb_pcm_pointer Awb_Block->u4WriteIdx;= 0x%x \n", I2S0_AWB_Block->u4WriteIdx);
    Auddrv_I2S0_AWB_Spinlock_lock();
    if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_AWB) == true)
    {
        ...
        if (HW_Cur_ReadIdx == 0)
        {
            pr_warn("[Auddrv] mtk_i2s0_awb_pcm_pointer  HW_Cur_ReadIdx ==0 \n");
            HW_Cur_ReadIdx = I2S0_AWB_Block->pucPhysBufAddr;
        }
        HW_memory_index = (HW_Cur_ReadIdx - I2S0_AWB_Block->pucPhysBufAddr);

        // update for data get to hardware
        Hw_Get_bytes = (HW_Cur_ReadIdx - I2S0_AWB_Block->pucPhysBufAddr) - I2S0_AWB_Block->u4WriteIdx;
        if (Hw_Get_bytes < 0)
        {
            Hw_Get_bytes += I2S0_AWB_Block->u4BufferSize;
        }
        I2S0_AWB_Block->u4WriteIdx   += Hw_Get_bytes;
        I2S0_AWB_Block->u4WriteIdx   %= I2S0_AWB_Block->u4BufferSize;
        I2S0_AWB_Block->u4DataRemained += Hw_Get_bytes;
        

        // buffer overflow
        if (I2S0_AWB_Block->u4DataRemained > I2S0_AWB_Block->u4BufferSize)
        {
            bIsOverflow = true;
            pr_warn("mtk_i2s0_awb_pcm_pointer buffer overflow u4DMAReadIdx:%x, u4WriteIdx:%x, u4DataRemained:%x, u4BufferSize:%x \n",
                   I2S0_AWB_Block->u4DMAReadIdx, I2S0_AWB_Block->u4WriteIdx, I2S0_AWB_Block->u4DataRemained, I2S0_AWB_Block->u4BufferSize);
        }
        
        //printk("[Auddrv] mtk_i2s0_awb_pcm_pointer =0x%x HW_memory_index = 0x%x\n", HW_Cur_ReadIdx, HW_memory_index);
        Auddrv_I2S0_AWB_Spinlock_unlock();
				      ...
				      struct snd_pcm_hw_params *hw_params)
{
    ...
    if (mCaptureUseSram == true)
    {
        ...
        pr_warn("mtk_i2s0_awb_pcm_hw_params mCaptureUseSram dma_bytes = %zu \n", runtime->dma_bytes);
        substream->runtime->dma_area = (unsigned char *)Get_Afe_SramBase_Pointer();
        substream->runtime->dma_addr = Get_Afe_Sram_Phys_Addr();
    }
    else if (Awb_Capture_dma_buf->area) {
...
static int mtk_i2s0_awb_pcm_open(struct snd_pcm_substream *substream)
{
...
#endif
    {
        pr_warn("mtk_i2s0_awb_pcm_open use sram 1\n");
        mtk_I2S0_awb_hardware.buffer_bytes_max = GetCaptureSramSize();
        SetSramState(SRAM_STATE_CAPTURE);
        mCaptureUseSram = true;
    }
    else
    {
        pr_warn("mtk_i2s0_awb_pcm_open use dram 2\n");
        mtk_I2S0_awb_hardware.buffer_bytes_max = UL1_MAX_BUFFER_SIZE;
    }
    ...
    //++ modified by shentaotao for system reboot when camera
    // here to open APLL
    ret = GetAPLLCounter();
    pr_warn("StartAudioCaptureHardware APLLCounter is %d\n", ret);
    if (ret >= 1) {
        for (i = 0; i < ret; i++) {
...
static int mtk_i2s0_dl1_awb_probe(struct snd_soc_platform *platform)
{
	...
	mAudioDigitalI2S0Awb =  kzalloc(sizeof(AudioDigtalI2S), GFP_KERNEL);
	return 0;
}
