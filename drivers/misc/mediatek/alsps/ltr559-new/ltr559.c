...
static int ltr559_ps_enable(struct i2c_client *client, int enable)
{
	...
	err = ltr559_i2c_write_reg(LTR559_PS_LED, 0x7b); 
	if(err<0)
	{
		APS_LOG("ltr559 set ps pulse error\n");
		return err;
	...
	//isadjust = 1;
		if((noise < last_min_value+500)){
			last_min_value = noise;
				if(noise < 100){
						atomic_set(&obj->ps_thd_val_high,  noise+70);//70
						atomic_set(&obj->ps_thd_val_low, noise+40); //50
				}else if(noise < 200){
...
static int ltr559_devinit(void)
{
	...
	res = ltr559_i2c_write_reg(LTR559_PS_LED, 0x7a); 
	if(res<0)
	{
		APS_LOG("ltr559 set ps pulse error\n");
		return res;
...
/*singal interrupt function add*/
		...
		if(intr_flag_value){
				...
				{
					if(obj->ps < 100){
						atomic_set(&obj->ps_thd_val_high,  obj->ps+70);//70
						atomic_set(&obj->ps_thd_val_low, obj->ps+40); //50
					}else if(obj->ps < 200){
