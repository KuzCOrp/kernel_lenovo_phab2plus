...
static int flightsense_local_init(void);
static int flightsense_remove(void);

static struct flightsense_init_info vl53l0_init_info = {
		.name = STMVL53L0_DRV_NAME,
		.init = flightsense_local_init,
		.uninit = flightsense_remove,
...
#define VL53L0_IOCTL_RANGE			_IO('p', 0x0e)
...
extern struct i2c_client *stmvl53l0_i2c_client;

#define STMVL53L0_CALIBRATION_TO_ISP

#if defined(STMVL53L0_CALIBRATION_TO_ISP)
unsigned int SpadCount_Calib=0;
uint8_t IsApertureSpads_Calib=0;
uint8_t VhvSettings_Calib=0;
uint8_t PhaseCal_Calib=0;
int Offset_Calib=0;
unsigned int XtalkInt_Calib=0;
uint16_t RangeMilliMeter_Calib=0;
unsigned int SignalRateRtnMegaCps_Calib=0;
#endif
...
static int stmvl53l0_write_isp(uint8_t category, uint8_t byte, uint8_t para)
{
	int err = 0;

	struct i2c_msg msg[1];
	struct i2c_client *client = stmvl53l0_i2c_client;
	uint8_t buffer[5] = {0};
	...
	if (err != 1)
	{
		pr_err("%s: err:%d, addr:0x%x\n", __func__, err, client->addr);
	}

	return 0;
}

static int stmvl53l0_read_isp(uint8_t category, uint8_t byte, uint8_t *buff)
{
	int err = 0;
...
#if defined(STMVL53L0_CALIBRATION_TO_ISP)
void stmvl53l0_read_calibration_to_isp(void)
{
	struct file *f;
	char buf[8];
	mm_segment_t fs;
	...
	f = filp_open("/data/calibration/SpadCount", O_RDONLY, 0);
	if (f != NULL && !IS_ERR(f) && f->f_dentry != NULL) {
		fs = get_fs();
		set_fs(get_ds());
		/* init the buffer with 0 */
		...
		f->f_op->read(f, buf, 8, &f->f_pos);
		set_fs(fs);
		vl53l0_dbgmsg("SpadCount as:%s, buf[0]:%c\n", buf, buf[0]);
		SpadCount_Calib = 0;
		for (i = 0; i < 8; i++) {
			if (i == 0 && buf[0] == '-')
				is_sign = 1;
			else if (buf[i] >= '0' && buf[i] <= '9')
				SpadCount_Calib = SpadCount_Calib * 10 +
					(buf[i] - '0');
			else
				break;
		}
		if (is_sign == 1)
			SpadCount_Calib = -SpadCount_Calib;
		vl53l0_dbgmsg("SpadCount_Calib as %d\n", SpadCount_Calib);

		filp_close(f, NULL);
	} else {
		vl53l0_errmsg("no SpadCount calibration file exist!\n");
	}

	is_sign = 0;
	f = filp_open("/data/calibration/IsApertureSpads", O_RDONLY, 0);
	if (f != NULL && !IS_ERR(f) && f->f_dentry != NULL) {
		fs = get_fs();
		set_fs(get_ds());
		/* init the buffer with 0 */
		...
		f->f_op->read(f, buf, 8, &f->f_pos);
		set_fs(fs);
		vl53l0_dbgmsg("IsApertureSpads as:%s, buf[0]:%c\n", buf, buf[0]);
		IsApertureSpads_Calib = 0;
		for (i = 0; i < 8; i++) {
			if (i == 0 && buf[0] == '-')
				is_sign = 1;
			else if (buf[i] >= '0' && buf[i] <= '9')
				IsApertureSpads_Calib = IsApertureSpads_Calib * 10 + (buf[i] - '0');
			else
				break;
		}
		if (is_sign == 1)
			IsApertureSpads_Calib = -IsApertureSpads_Calib;
		vl53l0_dbgmsg("IsApertureSpads_Calib as %d\n", IsApertureSpads_Calib);

		filp_close(f, NULL);
	} else {
		vl53l0_errmsg("no IsApertureSpads calibration file exist!\n");
	}

	is_sign = 0;
	f = filp_open("/data/calibration/VhvSettings", O_RDONLY, 0);
	if (f != NULL && !IS_ERR(f) && f->f_dentry != NULL) {
		fs = get_fs();
		set_fs(get_ds());
		/* init the buffer with 0 */
		...
		f->f_op->read(f, buf, 8, &f->f_pos);
		set_fs(fs);
		vl53l0_dbgmsg("VhvSettings as:%s, buf[0]:%c\n", buf, buf[0]);
		VhvSettings_Calib = 0;
		for (i = 0; i < 8; i++) {
			if (i == 0 && buf[0] == '-')
				is_sign = 1;
			else if (buf[i] >= '0' && buf[i] <= '9')
				VhvSettings_Calib = VhvSettings_Calib * 10 + (buf[i] - '0');
			else
				break;
		}
		if (is_sign == 1)
			VhvSettings_Calib = -VhvSettings_Calib;
		vl53l0_dbgmsg("VhvSettings_Calib as %d\n", VhvSettings_Calib);

		filp_close(f, NULL);
	} else {
		vl53l0_errmsg("no VhvSettings calibration file exist!\n");
	}

	is_sign = 0;
	f = filp_open("/data/calibration/PhaseCal", O_RDONLY, 0);
	if (f != NULL && !IS_ERR(f) && f->f_dentry != NULL) {
		fs = get_fs();
		set_fs(get_ds());
		/* init the buffer with 0 */
		...
		f->f_op->read(f, buf, 8, &f->f_pos);
		set_fs(fs);
		vl53l0_dbgmsg("PhaseCal as:%s, buf[0]:%c\n", buf, buf[0]);
		PhaseCal_Calib = 0;
		for (i = 0; i < 8; i++) {
			if (i == 0 && buf[0] == '-')
				is_sign = 1;
			else if (buf[i] >= '0' && buf[i] <= '9')
				PhaseCal_Calib = PhaseCal_Calib * 10 + (buf[i] - '0');
			else
				break;
		}
		if (is_sign == 1)
			PhaseCal_Calib = -PhaseCal_Calib;
		vl53l0_dbgmsg("PhaseCal_Calib as %d\n", PhaseCal_Calib);

		filp_close(f, NULL);
	} else {
		vl53l0_errmsg("no PhaseCal calibration file exist!\n");
	}
	...
	is_sign = 0;
	f = filp_open("/data/calibration/XtalkInt", O_RDONLY, 0);
	if (f != NULL && !IS_ERR(f) && f->f_dentry != NULL) {
		fs = get_fs();
		set_fs(get_ds());
		/* init the buffer with 0 */
		...
		f->f_op->read(f, buf, 8, &f->f_pos);
		set_fs(fs);
		vl53l0_dbgmsg("XtalkInt as:%s, buf[0]:%c\n", buf, buf[0]);
		XtalkInt_Calib = 0;
		for (i = 0; i < 8; i++) {
			if (i == 0 && buf[0] == '-')
				is_sign = 1;
			else if (buf[i] >= '0' && buf[i] <= '9')
				XtalkInt_Calib = XtalkInt_Calib * 10 + (buf[i] - '0');
			else
				break;
		}
		if (is_sign == 1)
			XtalkInt_Calib = -XtalkInt_Calib;
		vl53l0_dbgmsg("XtalkInt_Calib as %d\n", XtalkInt_Calib);

		filp_close(f, NULL);
	} else {
		vl53l0_errmsg("no XtalkInt calibration file exist!\n");
	}

	is_sign = 0;
	f = filp_open("/data/calibration/RangeMilliMeter", O_RDONLY, 0);
	if (f != NULL && !IS_ERR(f) && f->f_dentry != NULL) {
		fs = get_fs();
		set_fs(get_ds());
		/* init the buffer with 0 */
		...
		f->f_op->read(f, buf, 8, &f->f_pos);
		set_fs(fs);
		vl53l0_dbgmsg("RangeMilliMeter as:%s, buf[0]:%c\n", buf, buf[0]);
		RangeMilliMeter_Calib = 0;
		for (i = 0; i < 8; i++) {
			if (i == 0 && buf[0] == '-')
				is_sign = 1;
			else if (buf[i] >= '0' && buf[i] <= '9')
				RangeMilliMeter_Calib = RangeMilliMeter_Calib * 10 + (buf[i] - '0');
			else
				break;
		}
		if (is_sign == 1)
			RangeMilliMeter_Calib = -RangeMilliMeter_Calib;
		vl53l0_dbgmsg("RangeMilliMeter_Calib as %d\n", RangeMilliMeter_Calib);

		filp_close(f, NULL);
	} else {
		vl53l0_errmsg("no RangeMilliMeter calibration file exist!\n");
	}

	is_sign = 0;
	f = filp_open("/data/calibration/SignalRateRtnMegaCps", O_RDONLY, 0);
	if (f != NULL && !IS_ERR(f) && f->f_dentry != NULL) {
		fs = get_fs();
		set_fs(get_ds());
		/* init the buffer with 0 */
		...
		f->f_op->read(f, buf, 8, &f->f_pos);
		set_fs(fs);
		vl53l0_dbgmsg("SignalRateRtnMegaCps as:%s, buf[0]:%c\n", buf, buf[0]);
		SignalRateRtnMegaCps_Calib = 0;
		for (i = 0; i < 8; i++) {
			if (i == 0 && buf[0] == '-')
				is_sign = 1;
			else if (buf[i] >= '0' && buf[i] <= '9')
				SignalRateRtnMegaCps_Calib = SignalRateRtnMegaCps_Calib * 10 + (buf[i] - '0');
			else
				break;
		}
		if (is_sign == 1)
			SignalRateRtnMegaCps_Calib = -SignalRateRtnMegaCps_Calib;
		vl53l0_dbgmsg("SignalRateRtnMegaCps_Calib as %d\n", SignalRateRtnMegaCps_Calib);

		filp_close(f, NULL);
	} else {
		vl53l0_errmsg("no SignalRateRtnMegaCps calibration file exist!\n");
	}

	buffer[0] = (uint8_t) (SpadCount_Calib >> 24);
	buffer[1] = (uint8_t)((SpadCount_Calib &  0x00FF0000) >> 16);
	buffer[2] = (uint8_t)((SpadCount_Calib &  0x0000FF00) >> 8);
	buffer[3] = (uint8_t) (SpadCount_Calib &  0x000000FF);
	buffer[4] = IsApertureSpads_Calib;
	buffer[5] = VhvSettings_Calib;
	buffer[6] = PhaseCal_Calib;
	buffer[7] = (uint8_t) (Offset_Calib >> 24);
	buffer[8] = (uint8_t)((Offset_Calib &  0x00FF0000) >> 16);
	buffer[9] = (uint8_t)((Offset_Calib &  0x0000FF00) >> 8);
	buffer[10] = (uint8_t) (Offset_Calib &  0x000000FF);
	buffer[11] = (uint8_t) (XtalkInt_Calib >> 24);
	buffer[12] = (uint8_t)((XtalkInt_Calib &  0x00FF0000) >> 16);
	buffer[13] = (uint8_t)((XtalkInt_Calib &  0x0000FF00) >> 8);
	buffer[14] = (uint8_t) (XtalkInt_Calib &  0x000000FF);
	buffer[15] = (uint8_t)(RangeMilliMeter_Calib >> 8);
	buffer[16] = (uint8_t)(RangeMilliMeter_Calib &  0x00FF);
	buffer[17] = (uint8_t) (SignalRateRtnMegaCps_Calib >> 24);
	buffer[18] = (uint8_t)((SignalRateRtnMegaCps_Calib &  0x00FF0000) >> 16);
	buffer[19] = (uint8_t)((SignalRateRtnMegaCps_Calib &  0x0000FF00) >> 8);
	buffer[20] = (uint8_t) (SignalRateRtnMegaCps_Calib &  0x000000FF);

	for (i=0; i<21; i++)
	{
		vl53l0_dbgmsg("write buffer[%d]=%x\n", i, buffer[i]);
	}

	for (i=0; i<21; i++)
	{
		stmvl53l0_write_isp(0x0E, 0x8A+i, buffer[i]);
	}
	...
	for (i=0; i<21; i++)
	{
		stmvl53l0_read_isp(0x0E, 0x8A+i, buffer+i);
	}

	for (i=0; i<21; i++)
	{
		vl53l0_dbgmsg("read buffer[%d]=%x\n", i, buffer[i]);
	}

	stmvl53l0_write_isp(0x0D, 0x2D, 0x00);
}
#endif
...
*/
	...
	/* range */
	case VL53L0_IOCTL_RANGE:
		vl53l0_dbgmsg("VL53L0_IOCTL_RANGE\n");
		/* turn on tof sensor only if it's not enabled by other
		client */
		if (data->enable_ps_sensor == 0) {
			/* to start */
			stmvl53l0_start(data, 3, RANGE_MODE);
		} else
			rc = -EINVAL;
		break;
	/* disable */
	...
	init_mode_e mode)
{
	...
	{
		#if 1
		stmvl53l0_write_isp(0x0d, 0x72, 0x00);
		msleep(1);
		stmvl53l0_write_isp(0x0d, 0x72, 0x01);
		msleep(3);
		data->reset = 1;
		#else
		/* Power up */
...
"Could not register misc. dev for stmvl53l0	ranging\n");
	...
	data->useCase = USE_CASE_HIGH_ACCURACY;
	data->timingBudget = HIGH_ACCURACY_TIMING_BUDGET;
	
	/* Set default values used in Custom Mode Use Case */
	data->signalRateLimit = HIGH_ACCURACY_SIGNAL_RATE_LIMIT;
	data->sigmaLimit = HIGH_ACCURACY_SIGMA_LIMIT;
	data->preRangePulsePeriod = HIGH_ACCURACY_PRE_RANGE_PULSE_PERIOD;
	data->finalRangePulsePeriod = HIGH_ACCURACY_FINAL_RANGE_PULSE_PERIOD;
...
static int __init stmvl53l0_init(void)
{
	...
	flightsense_driver_add(&vl53l0_init_info);
