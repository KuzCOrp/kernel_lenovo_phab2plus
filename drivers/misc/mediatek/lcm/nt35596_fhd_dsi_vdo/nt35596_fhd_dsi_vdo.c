...
static unsigned int GPIO_LCD_BL_EN, GPIO_LCD_RST;
#ifndef CONFIG_FPGA_EARLY_PORTING
...
#define SET_RESET_PIN(pin, v)    (lcm_set_gpio_output((pin), (v)))
...
static void lcm_get_gpio_infor(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6753p1_64-lcm");

	GPIO_LCD_BL_EN = of_get_named_gpio(node, "gpio_lcm_bl_en", 0);
	GPIO_LCD_RST = of_get_named_gpio(node, "gpio_lcm_rst", 0);
#ifndef CONFIG_FPGA_EARLY_PORTING
#ifndef CONFIG_MTK_LEGACY
...
static void lcm_get_params(LCM_PARAMS *params)
{
       ...
       //1 SSD2075 has no TE Pin 
		...
		params->dsi.vertical_sync_active				= 2;//4;//1;  //---3
		params->dsi.vertical_backporch					= 4;//15;//9; //---14
		params->dsi.vertical_frontporch					= 4;//16;//8;  //----8
		params->dsi.vertical_active_line				= FRAME_HEIGHT; //1920

		params->dsi.horizontal_sync_active				= 4;//20;//4;  //----2
		params->dsi.horizontal_backporch				= 28;//56;//32; //----28
		params->dsi.horizontal_frontporch				= 28;//104;//32; //----50
...
//Added by Shentaotao for device info
#ifdef SLT_DEVINFO_LCM
	params->module = "LCD";
	params->vendor = "O-film";
	params->ic = "NT35596";
	params->info = "1920*1080";
#endif
}
...
static void lcm_init(void)
{
	...
	MDELAY(10);
	SET_RESET_PIN(GPIO_LCD_RST, 1);
	MDELAY(10);          //>1ms
	SET_RESET_PIN(GPIO_LCD_RST, 0);
	MDELAY(20);          //>10us
