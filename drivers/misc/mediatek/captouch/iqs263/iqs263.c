...
static void iqs263_eint_work(struct work_struct *work)
{
	...
	{
		err = iqs263_i2c_read_dma(iqs263_i2c_client, 0x0a, 8, buffer);
	...
	else if (1 == captouch_init_flag)
	{
		...
		buffer[6] = 0xff;
		buffer[7] = 0x0f;
		err = iqs263_i2c_write_dma(iqs263_i2c_client, 0x0a, 8, buffer);

		captouch_init_flag = 0;
	}
#endif
	...
	if (0 == captouch_init_flag)
	{
		buffer[0] = 0x07;
		#if defined(IQS263_SUPPORT_I2C_DMA)
		err = iqs263_i2c_write_dma(iqs263_i2c_client, 0x0d, 1, buffer);
		#else
		err = iqs263_i2c_write(iqs263_i2c_client, 0x0d, 1, buffer);
		#endif
		if (err >= 0)
		{
			captouch_init_flag = 1;
	...
	else if (1 == captouch_init_flag)
	{
		...
		err = iqs263_i2c_write_dma(iqs263_i2c_client, 0x0a, 8, buffer);
		#else
		err = iqs263_i2c_write(iqs263_i2c_client, 0x0a, 7, buffer);
		#endif
		if (err >= 0)
		{
	...
	else if (2 == captouch_init_flag)
	{
		...
		buffer[2] = 0xC8;
		#if defined(IQS263_SUPPORT_I2C_DMA)
		err = iqs263_i2c_write_dma(iqs263_i2c_client, 0x0b, 3, buffer);
		#else
		err = iqs263_i2c_write(iqs263_i2c_client, 0x0b, 3, buffer);
		#endif
		if (err >= 0)
		{
	...
	else if (3 == captouch_init_flag)
	{
		...
		buffer[4] = 0x11;
		#if defined(IQS263_SUPPORT_I2C_DMA)
		err = iqs263_i2c_write_dma(iqs263_i2c_client, 0x07, 5, buffer);
		#else
		err = iqs263_i2c_write(iqs263_i2c_client, 0x07, 5, buffer);
		#endif
		if (err >= 0)
		{
	...
	else if (4 == captouch_init_flag)
	{
		...
		buffer[4] = 0x02;
		#if defined(IQS263_SUPPORT_I2C_DMA)
		err = iqs263_i2c_write_dma(iqs263_i2c_client, 0x09, 5, buffer);
		#else
		err = iqs263_i2c_write(iqs263_i2c_client, 0x09, 5, buffer);
		#endif
		if (err >= 0)
		{
	...
	{
		#if defined(IQS263_SUPPORT_I2C_DMA)
		err = iqs263_i2c_read_dma(iqs263_i2c_client, 0x03, 1, buffer);
		#else
...
/*----------------------------------------------------------------------------*/
static ssize_t iqs263_show_disable(struct device_driver *ddri, char *buf)
{
	CAPTOUCH_LOG("iqs263_show_disable \n");
...
static DRIVER_ATTR(iqs263_disable, S_IWUSR | S_IRUGO, iqs263_show_disable, NULL);

/*----------------------------------------------------------------------------*/
static struct driver_attribute *iqs263_attr_list[] =
{
	&driver_attr_iqs263_enable,
	&driver_attr_iqs263_disable,
};
/*----------------------------------------------------------------------------*/
...
#if defined(CONFIG_MTK_IQS263)
extern volatile int call_status;
static int iqs263_enable_flag = 0;
void iqs263_suspend(void)
{
	CAPTOUCH_FUN();
	if (!call_status)
	{
		disable_irq(captouch_irq);
		iqs263_enable_flag = 1;
	}
	else
	{
		iqs263_enable_flag = 0;
	}
}
/*----------------------------------------------------------------------------*/
void iqs263_resume(void)
{
	CAPTOUCH_FUN();
	if (iqs263_enable_flag)
		enable_irq(captouch_irq);
}
#endif
//-- Modified for close Captouch by shentaotao 2016.07.02
