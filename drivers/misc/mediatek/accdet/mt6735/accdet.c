...
#if 1 //modified by zhangtao 20150127
volatile int call_status =0;
#else
...
/*Disable ACCDET*/
	...
	if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		if ((ts3a227e_reg_value[11] & 0x01) == 0x01) {
			ACCDET_DEBUG("[Accdet] TS3A227E 3-pole headset detected!\n");
			ts3a227e_connector_type = TS3A227E_CONNECTOR_TRS;
			ts3a227e_write_byte(0x04, 0x48);
			ts3a227e_write_byte(0x07, 0x10);
			ts3a227e_write_byte(0x08, 0x09);
			msleep(20);
		} else if ((ts3a227e_reg_value[11] & 0x02) == 0x02) {
			ACCDET_DEBUG("[Accdet] TS3A227E 4-pole OMTP headset detected!\n");
			ts3a227e_connector_type = TS3A227E_CONNECTOR_TRRS_OMTP;
		} else if ((ts3a227e_reg_value[11] & 0x04) == 0x04) {
			ACCDET_DEBUG("[Accdet] TS3A227E 4-pole standard headset detected!\n");
			ts3a227e_connector_type = TS3A227E_CONNECTOR_TRRS_STANDARD;
			
		} else {
			ACCDET_DEBUG("[Accdet] TS3A227E Detection sequence completed without successful!\n");
		}
#endif
...
#else
		mt_gpio_set_debounce(gpiopin, accdet_dts_data.accdet_plugout_debounce * 1000);
#endif
		/* update the eint status */
...
static ssize_t show_TS3A227EConnectorType_Set_One(struct device_driver *ddri, char *buf)
{
	ts3a227e_connector_type = 1;
	return sprintf(buf, "%u\n", ts3a227e_connector_type);
}

static DRIVER_ATTR(TS3A227EConnectorType_Set_One, 0444, show_TS3A227EConnectorType_Set_One, NULL);

static ssize_t show_TS3A227EConnectorType_Set_Two(struct device_driver *ddri, char *buf)
{
	ts3a227e_connector_type = 2;
	return sprintf(buf, "%u\n", ts3a227e_connector_type);
}

static DRIVER_ATTR(TS3A227EConnectorType_Set_Two, 0444, show_TS3A227EConnectorType_Set_Two, NULL);

static ssize_t show_TS3A227EOnS3GSR(struct device_driver *ddri, char *buf)
{
	ACCDET_DEBUG("[Accdet] TS3A227EOnS3GSR ts3a227e_connector_type=%d\n", ts3a227e_connector_type);

	if (ts3a227e_connector_type == TS3A227E_CONNECTOR_TRRS_OMTP)
	{
		ts3a227e_write_byte(0x04, 0x48);
		ts3a227e_write_byte(0x07, 0x20);
		ts3a227e_write_byte(0x08, 0x06);
	}
	else if (ts3a227e_connector_type == TS3A227E_CONNECTOR_TRRS_STANDARD)
	{
		ts3a227e_write_byte(0x04, 0x48);
		ts3a227e_write_byte(0x07, 0x10);
		ts3a227e_write_byte(0x08, 0x09);
...
static DRIVER_ATTR(TS3A227EOnS3GSR, 0664, show_TS3A227EOnS3GSR, NULL);

static ssize_t show_TS3A227EOffS3GSR(struct device_driver *ddri, char *buf)
{
	ACCDET_DEBUG("[Accdet] TS3A227EOffS3GSR ts3a227e_connector_type=%d\n", ts3a227e_connector_type);
...
static DRIVER_ATTR(TS3A227EOffS3GSR, 0664, show_TS3A227EOffS3GSR, NULL);

//++Added by shentaotao 2016.06.07
int ts3a227e_connector_type_notify = 0;
static ssize_t show_TS3A227EConnectorType_Notify(struct device_driver *ddri, char *buf)
{
	return sprintf(buf, "%d\n", ts3a227e_connector_type_notify);
}

static ssize_t store_TS3A227EConnectorType_Notify(struct device_driver *ddri, const char *buf, size_t count)
{
	int flag = 0;

	if (!kstrtoint(buf, 10, &flag)) {
		ts3a227e_connector_type_notify = flag;
	}

	printk("STT: ts3a227e_connector_type_notify = %d\n", ts3a227e_connector_type_notify);
	return count;
}

static DRIVER_ATTR(TS3A227EConnectorType_Notify, 0664, show_TS3A227EConnectorType_Notify, store_TS3A227EConnectorType_Notify);
//--Added by shentaotao 2016.06.07
#endif

#if 1	//modified by zhangtao 20150127
static ssize_t accdet_show_call_state(struct device_driver *ddri, char *buf)
{
	ACCDET_DEBUG("[Accdet] call_status=%d\n", call_status);
	return sprintf(buf, "%u\n", call_status);
}
#endif
...
#if 1	//modified by zhangtao 20150127
static DRIVER_ATTR(accdet_call_state, 0664, accdet_show_call_state, accdet_store_call_state);
#else
...
static struct driver_attribute *accdet_attr_list[] = {
	...
	&driver_attr_TS3A227EConnectorType_Set_One,
	&driver_attr_TS3A227EConnectorType_Set_Two,
	&driver_attr_TS3A227EOnS3GSR,
	&driver_attr_TS3A227EOffS3GSR,
	&driver_attr_TS3A227EConnectorType_Notify,
#endif
};
