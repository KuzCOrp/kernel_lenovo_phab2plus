...
extern void ultra_16_hal(u32 bit);
extern void ultra_24_hal(u32 bit);
extern void ultra_4_hal(u32 bit);
extern void mt_set_pwm_delay_clock_hal (u32 pwm_no, u32 clksrc);
extern void mt_set_pwm_enable_ahb_hal(u32 bit);
extern void mt_set_pwm_delay_duration_hal(u32 pwm_no, u16 val);
extern void mt_set_pwm_seqmode_multiport_enable_hal(u32 pwm_no_sclk, u32 pwm_no_mosi);
...
extern void mt_set_pwm_buf1_addr_hal (u32 pwm_no, u32 addr);
extern void mt_set_pwm_buf1_size_hal( u32 pwm_no, u16 size);
extern void mt_set_pwm_valid_hal ( u32 pwm_no, u32 buf_valid_bit );
extern void mt_set_pwm_buf1_addr_hal (u32 pwm_no, u32 addr);
...
s32 mt_set_pwm_seqmode_multiport_enable(u32 pwm_no_sclk, u32 pwm_no_mosi) 
{
	unsigned long flags;
...
s32 pwm_set_spec_config_for_spi(struct pwm_spec_config *conf_sclk, struct pwm_spec_config *conf_mosi)
{
	...
	//printk(" conf_sclk->pmic_pad(%d),conf_mosi->pmic_pad(%d)\n",  conf_sclk->pmic_pad, conf_mosi->pmic_pad);
...
//}else if ( conf_mosi->mode == PWM_MODE_FIFO) {
#if 1
	//mt_set_pwm_disable(conf_mosi->pwm_no);
	mt_set_pwm_con_oldmode(conf_mosi->pwm_no, OLDMODE_DISABLE);
	mt_set_pwm_con_datasrc(conf_mosi->pwm_no, MEMORY);
...
#else
//pwm_set_spec_config(conf_mosi);

//mt_set_pwm_disable(conf_mosi->pwm_no);
mt_set_pwm_con_oldmode(conf_mosi->pwm_no, OLDMODE_DISABLE);
mt_set_pwm_con_datasrc(conf_mosi->pwm_no, PWM_FIFO);
mt_set_pwm_con_mode (conf_mosi->pwm_no, PERIOD);
mt_set_pwm_con_idleval(conf_mosi->pwm_no, conf_mosi->PWM_MODE_FIFO_REGS.IDLE_VALUE);
mt_set_pwm_con_guardval (conf_mosi->pwm_no, conf_mosi->PWM_MODE_FIFO_REGS.GUARD_VALUE);
mt_set_pwm_HiDur (conf_mosi->pwm_no, conf_mosi->PWM_MODE_FIFO_REGS.HDURATION);
mt_set_pwm_LowDur (conf_mosi->pwm_no, conf_mosi->PWM_MODE_FIFO_REGS.LDURATION);
mt_set_pwm_GuardDur (conf_mosi->pwm_no, conf_mosi->PWM_MODE_FIFO_REGS.GDURATION);
mt_set_pwm_send_data0(conf_mosi->pwm_no, conf_mosi->PWM_MODE_FIFO_REGS.SEND_DATA0);
mt_set_pwm_send_data1(conf_mosi->pwm_no, conf_mosi->PWM_MODE_FIFO_REGS.SEND_DATA1);
mt_set_pwm_wave_num(conf_mosi->pwm_no, conf_mosi->PWM_MODE_FIFO_REGS.WAVE_NUM);
mt_set_pwm_con_stpbit(conf_mosi->pwm_no, conf_mosi->PWM_MODE_FIFO_REGS.STOP_BITPOS_VALUE,PWM_FIFO);
#endif
}
...
//printk("mt_get_gpio_mode(59|0x80000000): %d\n" ,mt_get_gpio_mode(59|0x80000000));
//printk("mt_get_gpio_mode(21|0x80000000): %d\n" ,mt_get_gpio_mode(21|0x80000000));

	//enable pwm
	mt_set_pwm_seqmode_multiport_enable(conf_sclk->pwm_no, conf_mosi->pwm_no);
	//mt_set_pwm_enable(conf_sclk->pwm_no);
	//mt_set_pwm_enable(conf_mosi->pwm_no);
	...
	count = 0;
	while (!mt_get_intr_status(4))
	{
		udelay(10);
		count++;
		if (count > 100000) {
			printk("timout mt_get_intr_status(0)\n");
			break;
		}
	}

	count = 0;
	while (!mt_get_intr_status(6))
	{
		udelay(10);
		count++;
		if (count > 100000) {
			printk("timout mt_get_intr_status(8)\n");
			break;
		}
	}
	...
	//printk("int finish status_0 : %lx\n", reg_val);
	reg_val = mt_get_intr_status(5);
	//printk("int underflow status_1 : %lx\n", reg_val);

	reg_val = mt_get_intr_status(6);
	//printk("int finish status_8 : %lx\n", reg_val);
	reg_val = mt_get_intr_status(7);
	//printk("int underflow status_9 : %lx\n", reg_val);

//	pr_err("bus_control0_2 = : %x\n", INREG32(PWM_ULTRA));
    mt_set_intr_ack(4);
	mt_set_intr_ack(5);
	mt_set_intr_ack(6);
	mt_set_intr_ack(7);
