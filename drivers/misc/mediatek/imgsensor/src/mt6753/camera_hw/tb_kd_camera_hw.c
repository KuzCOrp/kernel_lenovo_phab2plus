...
#define PK_DBG(fmt, args...)        pr_info(PFX  fmt, ##args)
...
struct pinctrl_state *cam_spi_cs_h = NULL;
struct pinctrl_state *cam_spi_cs_l = NULL;
...
//spi cs pin control
	cam_spi_cs_h = pinctrl_lookup_state(camctrl, "cam_spi_cs_1");
	if (IS_ERR(cam_spi_cs_h)) {
		ret = PTR_ERR(cam_spi_cs_h);
		pr_debug("%s : pinctrl err, cam_spi_cs_h\n", __func__);
	}


	cam_spi_cs_l = pinctrl_lookup_state(camctrl, "cam_spi_cs_0");
	if (IS_ERR(cam_spi_cs_l)) {
		ret = PTR_ERR(cam_spi_cs_l);
		pr_debug("%s : pinctrl err, cam_spi_cs_l\n", __func__);
	}
	return ret;
}
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	case CAMSPICS:
		if (Val == 0)
			pinctrl_select_state(camctrl, cam_spi_cs_l);
		else
			pinctrl_select_state(camctrl, cam_spi_cs_h);
		break;
...
#define VOL_1000 1000000
	...
	if (On) {
        ...
        else if (currSensorName && ((0 == strcmp(currSensorName, SENSOR_DRVNAME_M12MO_YUV)) || ((0 == strcmp(currSensorName, SENSOR_DRVNAME_M12MO_SUBSENSOR_YUV))))) {
			
			if((pinSetIdx== 0) && (0 == strcmp(currSensorName, SENSOR_DRVNAME_M12MO_YUV)))
				{
					M12MO_Set_CameraMode(M12MO_MAIN_CAMERA);
				}
			else if((pinSetIdx== 1) && (0 == strcmp(currSensorName, SENSOR_DRVNAME_M12MO_SUBSENSOR_YUV)))
				{
					M12MO_Set_CameraMode(M12MO_SUB_CAMERA);
				}
			else
				{
					M12MO_Set_CameraMode(M12MO_ERROR_CAMERA);
				}
			/* First Power Pin low and Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
				mtkcam_gpio_set(pinSetIdx, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
			mtkcam_gpio_set(pinSetIdx, CAMSPICS, 1);
            		mdelay(5);
			mtkcam_gpio_set(pinSetIdx, CAMLDO1V2, 1);
			mtkcam_gpio_set(pinSetIdx, CAMLDO1V8, 1);
           		 mdelay(5);
				 
			ISP_MCLK1_EN(1);

			PK_DBG("[PowerON MCLK1]pinSetIdx:%d\n", pinSetIdx);
           		// mdelay(5);
			if (TRUE != _hwPowerOn(VCAMIO, VOL_1800)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable IO power (VCAM_IO), power id = %d\n", VCAMIO);
				goto _kdCISModulePowerOn_exit_;
        ...
        else if ( currSensorName && ((0 == strcmp(currSensorName, SENSOR_DRVNAME_M12MO_YUV)) || ((0 == strcmp(currSensorName, SENSOR_DRVNAME_M12MO_SUBSENSOR_YUV)))))
		{
			...
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
				mtkcam_gpio_set(pinSetIdx, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
           		 ...
           		 mdelay(1);
			mtkcam_gpio_set(pinSetIdx, CAMSPICS, 0);
			mtkcam_gpio_set(pinSetIdx, CAMLDO1V2, 0);
			mtkcam_gpio_set(pinSetIdx, CAMLDO1V8, 0);
           		 mdelay(1);
			if (TRUE != _hwPowerDown(VCAMD)) {
				PK_DBG("[CAMERA SENSOR] Fail to OFF digital power (VCAM_IO),power id = %d\n", VCAMD);
				/* return -EIO; */
