		...
		struct spi_ioc_transfer *u_xfers, unsigned n_xfers)
{
	...
	memset(rx_buf, 0x5a, u_xfers->len);//liulei
	...
	rx_buf = spidev->rx_buffer;
//	printk("u_tmp->len(%d), k_tmp->len(%d), u_xfers->len(%d)\n", u_tmp->len, k_tmp->len, u_xfers->len);
	if (rx_buf)
		debug_packet(&spidev->spi->dev, "rx xfer", rx_buf, u_xfers->len);
	else
		printk("rx xfer is null\n");
