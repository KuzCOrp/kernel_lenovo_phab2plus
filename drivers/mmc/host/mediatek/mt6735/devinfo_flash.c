...
{	
	...
	char *type, *module, *vendor, *ic, *version;
	char *info = kmalloc(64,GFP_KERNEL);	//RAM SIZE
	char *info1 = kmalloc(64,GFP_KERNEL);	//RAM SIZE

#ifdef DEVINFO_DEBUG_EMCP
    printk("[DEVINFO_EMCP][%s]: register emcp info.[%d][%x]\n", __func__, num_of_emi_records, host->mmc->card->type);
    ...
    printk("[DEVINFO_EMCP]: emcp list info.[%x][%x][%x][%x][%x][%x][%x][%x]\n", emi_settings[i].ID[0],
		emi_settings[i].ID[1], emi_settings[i].ID[2], emi_settings[i].ID[3], emi_settings[i].ID[4],
		emi_settings[i].ID[5], emi_settings[i].ID[6], emi_settings[i].ID[7]);
#endif
		...
		ic = module;
#ifdef DEVINFO_DEBUG_EMCP
    	printk("[DEVINFO_EMCP]: device vendor:[%s],device ic:[%s]!\n",vendor);
		...
		else
		{
			...
			if((emi_settings[i].ID[0] == (host->mmc->card->raw_cid[0] & 0xFF000000)>>24) &&
				(emi_settings[i].ID[1] == (host->mmc->card->raw_cid[0] & 0x00FF0000)>>16) &&
				(emi_settings[i].ID[2] == (host->mmc->card->raw_cid[0] & 0x0000FF00)>>8) &&
				(emi_settings[i].ID[3] == (host->mmc->card->raw_cid[0] & 0x000000FF)>>0) &&
				(emi_settings[i].ID[4] == (host->mmc->card->raw_cid[1] & 0xFF000000)>>24) &&
				(emi_settings[i].ID[5] == (host->mmc->card->raw_cid[1] & 0x00FF0000)>>16) &&
				(emi_settings[i].ID[6] == (host->mmc->card->raw_cid[1] & 0x0000FF00)>>8))
			{
				switch(ram_size)
				{
					case 2048:
...
#ifdef DEVINFO_DEBUG_EMCP
				printk("[DEVINFO_EMCP]: Get right device,Device info:%s\n", info);
#endif
...
#ifdef DEVINFO_DEBUG_EMCP
				printk("[DEVINFO_EMCP]: Get wrong device, Device info1:%s\n", info1);
#endif
...
static void devinfo_flash_localinit(void)
{
	...
	rom_size = msdc_get_user_capacity(host_ctl)/1024/1024;
	devinfo_register_emcp(host_ctl);

#ifdef DEVINFO_DEBUG_EMCP
	printk("[DEVINFO_EMCP]csd:0x%x,0x%x,0x%x,0x%x\n",host_ctl->mmc->card->raw_cid[0], host_ctl->mmc->card->raw_cid[1],
