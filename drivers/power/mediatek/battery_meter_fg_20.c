...
#define DIFFERENCE_SWOCV_RTC		25	/* shulei modify 10% to 25% difference */
#endif
...
#define BATTERY_ID_GPIO 60
#define MTK_GET_BATTERY_ID_BY_GPIO
#define LCT_MULTI_BAT_PROFILE_SUPPORT
#if 1
unsigned int g_fg_battery_id = 0;

#ifdef LCT_MULTI_BAT_PROFILE_SUPPORT
...
void fgauge_get_profile_id(void)
{
	...
	value = gpio_get_value(BATTERY_ID_GPIO);

	printk("STT: battery_id = %d\n", value);
	g_fg_battery_id = (unsigned int)value;
}
#else
void fgauge_get_profile_id(void)
{
...
#if defined(LCT_MULTI_BAT_PROFILE_SUPPORT)
static void __batt_meter_parse_qmax_node(const struct device_node *np,
				const char *node_srting, int *cust_val)
{
	u32 val1,val2;
	if (g_fg_battery_id == 1)
	{
		if (of_property_read_u32_index(np, node_srting, 0, &val1) == 0) {
			(*cust_val) = (int)val1;
		...
		if (of_property_read_u32_index(np, node_srting, 1, &val2) == 0) {
			(*cust_val) = (int)val2;
			bm_print(BM_LOG_FULL, "Get %s: %d\n", node_srting, (*cust_val));
		} else {
			bm_print(BM_LOG_CRTI, "Get %s failed\n", node_srting);
				...
				const char *node_srting, int iSize, BATTERY_PROFILE_STRUCT_P profile_p)
{
	...
	if(g_fg_battery_id == 1){
		...
		msleep(20);
		bm_print(BM_LOG_CRTI, "__batt_meter_parse_table>> %s[%d]: <%d, %d>\n",
				node_srting, ((idx-154)/2), profile_p->percentage, profile_p->voltage);
		#endif
...
int __batt_meter_init_cust_data_from_dt(void)
{
	...
	else if(g_fg_battery_id == 0)
	{
		...
		while (!of_property_read_u32_index(np, "batt_temperature_table", idx, &addr)) {
			...
			Batt_Temperature_Table[idx / 2-17].BatteryTemp = addr;
			Batt_Temperature_Table[idx / 2-17].TemperatureR = val;
...
#if defined(LCT_MULTI_BAT_PROFILE_SUPPORT)
	__batt_meter_parse_qmax_node(np, "q_max_pos_50",
		&batt_meter_cust_data.q_max_pos_50);

	__batt_meter_parse_qmax_node(np, "q_max_pos_25",
		&batt_meter_cust_data.q_max_pos_25);

	__batt_meter_parse_qmax_node(np, "q_max_pos_0",
		&batt_meter_cust_data.q_max_pos_0);

	__batt_meter_parse_qmax_node(np, "q_max_neg_10",
		&batt_meter_cust_data.q_max_neg_10);

	__batt_meter_parse_qmax_node(np, "q_max_pos_50_h_current",
		&batt_meter_cust_data.q_max_pos_50_h_current);

	__batt_meter_parse_qmax_node(np, "q_max_pos_25_h_current",
		&batt_meter_cust_data.q_max_pos_25_h_current);

	__batt_meter_parse_qmax_node(np, "q_max_pos_0_h_current",
		&batt_meter_cust_data.q_max_pos_0_h_current);

	__batt_meter_parse_qmax_node(np, "q_max_neg_10_h_current",
...
#if defined(LCT_MULTI_BAT_PROFILE_SUPPORT)
	__batt_meter_parse_qmax_node(np, "batterypseudo100",
		&batt_meter_cust_data.batterypseudo100);

	__batt_meter_parse_qmax_node(np, "batterypseudo1",
...
#if defined(LCT_MULTI_BAT_PROFILE_SUPPORT)
static ssize_t g_fg_battery_id_show(struct device *dev,struct device_attribute *attr, char *buf)
{
    bm_print(BM_LOG_CRTI, "[FG] g_fg_battery_id : %d\n", g_fg_battery_id);
    return sprintf(buf, "%d\n", g_fg_battery_id);
}
static DEVICE_ATTR(FG_g_fg_battery_id, 0444, g_fg_battery_id_show, NULL);
#endif
...
#if defined(LCT_MULTI_BAT_PROFILE_SUPPORT)
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_FG_g_fg_battery_id);
#endif
