...
extern void mt_usb_bat_set_vbus(void);
void check_ACA_power(void){

	printk("[%s]\r\n", __func__);
	hw_otg_reset_aca_type();
	if((0x0 == hw_otg_get_aca_type())) {
		mt_usb_bat_set_vbus();
	}
}
#endif
...
kal_bool upmu_is_chr_det(void)
{
...
#if defined(CONFIG_THUB_SUPPORT)
	static int ACA_type = 0xF;
	static int ACA_debounce = 0x3;
#endif
	...
	if((0x4 == hw_otg_get_aca_type()) && (0x0 == ACA_type))
		ACA_wake_up_bat();
...
#if defined(CONFIG_THUB_SUPPORT)
	if (mt_usb_is_device()) {
		if(ACA_debounce && 0x8 == ACA_type) {
			/* add debounce 3 times to reset aca type*/
			ACA_debounce--;
			hw_otg_reset_aca_type();
			ACA_wake_up_bat();
		}
		...
		return KAL_TRUE;
	} else if (0x4 == ACA_type) {
		printk("[upmu_is_chr_det] ACA Cable plug in\n");
...
#if defined(MTK_TEMPERATURE_RECHARGE_SUPPORT)
static ssize_t show_BatteryTemperature(struct device *dev,struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_CRTI, "[Battery] show_BatteryTemperature : %d\n", g_battery_temperature);

	return sprintf(buf, "%d\n", g_battery_temperature);
}
static ssize_t store_BatteryTemperature(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	char *pvalue = NULL;
	unsigned int battery_temperature= 0;
	battery_log(BAT_LOG_CRTI, "[Battery] store_BatteryTemperature\n");
	
	if(buf != NULL && size != 0)
	{
		battery_log(BAT_LOG_CRTI, "[Battery] buf is %s \n",buf);
		battery_temperature = simple_strtol(buf,&pvalue,10);
...
static DEVICE_ATTR(BatteryTemperature, 0664, show_BatteryTemperature, store_BatteryTemperature);
#endif
...
PMU_STATUS do_batt_soc_state_machine(void)
{
	...
	{
		if (BMT_status.UI_SOC2 >= batt_cust_data.max_charge_soc)
		{
		...
		{
			if (BMT_status.UI_SOC2 <= batt_cust_data.over_soc_recharge_percent)
			{             
				battery_log(BAT_LOG_CRTI, "[BATTERY] Battery Temperature down from %d to %d(%d), allow charging!!\n\r", MAX_CHARGE_SOC,BMT_status.UI_SOC2,OVER_SOC_RECHARGE_PERCENT); 
...
void mt_battery_GetBatteryData(void)
{
	...
	#if 1	//modified by zhangtao 20151125
	if ((BMT_status.temperature>10) && (BMT_status.temperature<=45))
	{
		BMT_status.ICharging = 1984;
	}
	else
	{
		BMT_status.ICharging = 1536;
	}
	#endif
...
static PMU_STATUS mt_battery_CheckBatterySOC(void)
{
	...
	if (batt_cust_data.bat_over_soc_recharge)
	{
	...
	{
		if (BMT_status.UI_SOC2 >= MAX_CHARGE_SOC)
		{
			battery_log(BAT_LOG_CRTI, "[BATTERY] Battery Over SOC !!\n\r");
			status = PMU_STATUS_FAIL;
		}
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
	...
	//unsigned int hv_voltage = V_CHARGER_MAX * 1000;	//wuqingya modify for ALPS0272151
	kal_bool hv_status;
...
int __batt_init_cust_data_from_cust_header(void)
{
	...
	*/
	...
	/* Battery Over OSC Protection */
#if defined(BAT_OVER_SOC_RECHARGE)
	batt_cust_data.bat_over_soc_recharge = 1;
#endif

#if defined(MAX_CHARGE_SOC)
	batt_cust_data.max_charge_soc = MAX_CHARGE_SOC;
#endif

#if defined(OVER_SOC_RECHARGE_PERCENT)
	batt_cust_data.over_soc_recharge_percent = OVER_SOC_RECHARGE_PERCENT;
#endif
...
#endif
#if defined(MTK_TEMPERATURE_RECHARGE_SUPPORT)
	    ret_device_file = device_create_file(&(dev->dev), &dev_attr_BatteryTemperature);
#endif
	}
