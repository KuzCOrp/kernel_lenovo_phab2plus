...
static void devinfo_ctp_regchar(char *module,char * vendor,char *version,char *config,char *used)
{
	...
	s_DEVINFO_ctp->device_ic = "GT9271";
	s_DEVINFO_ctp->device_info = config;
...
#if defined(LCT_ADD_TP_VERSION)
static ssize_t gtp_info_read_proc(struct file *file, char *buffer, size_t count,  loff_t *ppos)
{
	char *page = NULL;
	char *ptr = NULL;
	...
	ptr = page;
	ptr += sprintf(ptr, "[Vendor]%s, [FW]0x%04x, [IC]GT9271\n", manu_string, tpd_info.vid);
...
s32 gtp_send_cfg(struct i2c_client *client)
{
	...
	s32 retry = 0;
	if(glove_tpd_halt) {
		GTP_INFO("TP suspend , no config send");
		return 0;	
	}
	if (fixed_config) {
	...
	for (retry = 0; retry < 5; retry++) {
		...
		{
			GTP_DEBUG("HALL near, gtp_glove_mode_status= %d\n", gtp_glove_mode_status);
...
#endif
		{
//		printk("HALL away, gtp_glove_mode_status= %d\n", gtp_glove_mode_status);
			mutex_lock(&i2c_access);
...
static s32 gtp_init_panel(struct i2c_client *client)
{
...
#if defined(CONFIG_GTP_GLOVE_MODE)
	const u8 cfg_grp1_glove[] = GTP_CFG_GROUP1_GLOVE;
	const u8 cfg_grp2_glove[] = GTP_CFG_GROUP2_GLOVE;
	const u8 cfg_grp3_glove[] = GTP_CFG_GROUP3_GLOVE;
	const u8 cfg_grp4_glove[] = GTP_CFG_GROUP4_GLOVE;
	const u8 cfg_grp5_glove[] = GTP_CFG_GROUP5_GLOVE;
	const u8 cfg_grp6_glove[] = GTP_CFG_GROUP6_GLOVE;
	const u8 *cfgs_glove[] = {
		cfg_grp1_glove, cfg_grp2_glove, cfg_grp3_glove,
		cfg_grp4_glove, cfg_grp5_glove, cfg_grp6_glove
	};
	u8 cfg_lens_glove[] = {
		CFG_GROUP_LEN(cfg_grp1_glove),
		CFG_GROUP_LEN(cfg_grp2_glove),
		CFG_GROUP_LEN(cfg_grp3_glove),
		CFG_GROUP_LEN(cfg_grp4_glove),
		CFG_GROUP_LEN(cfg_grp5_glove),
		CFG_GROUP_LEN(cfg_grp6_glove)};
#endif
...
#if defined(CONFIG_GTP_GLOVE_MODE)
	GTP_DEBUG("Charger Config Groups Length: %d, %d, %d, %d, %d, %d", cfg_lens_glove[0],
		  cfg_lens_glove[1], cfg_lens_glove[2], cfg_lens_glove[3], cfg_lens_glove[4], cfg_lens_glove[5]);

	memset(&gtp_glove_config[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
	if (cfg_lens_glove[sensor_id] == cfg_len)
		memcpy(&gtp_glove_config[GTP_ADDR_LENGTH], cfgs_glove[sensor_id], cfg_len);
	...
	gtp_glove_config[cfg_len] = (~check_sum) + 1;
#endif
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	switch (tpd_info.sid)
	{
		case 0:
			...
			break;
		case 2:
			devinfo_ctp_regchar("IntelS", "IntelS", temp_ver, temp_ver_config, DEVINFO_USED);
			break;
		default:
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		if (DOZE_ENABLED == doze_status) {
			...
			GTP_DEBUG("doze_buf[4]=%d, doze_buf[5]=%d, doze_buf[6]=%d, doze_buf[7]=%d\n",
				doze_buf[4], doze_buf[5], doze_buf[6], doze_buf[7]);
			if (ret > 0) {
				if ((doze_buf[7] == 0) && (doze_buf[6] == 0) && (doze_buf[5] == 0) &&
					((doze_buf[4] == 1) || (doze_buf[4] == 2) || (doze_buf[4] == 4)))
				{
					/* clear 0x814B */
					doze_buf[2] = 0x00;
