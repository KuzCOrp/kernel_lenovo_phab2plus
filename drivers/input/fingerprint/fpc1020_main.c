...
#define DEBUG
#define CONFIG_PINCTRL_MT6753
#define FPC_ADD_FPC1020_NODE
//#define MANUAL_CS
...
#if defined(FPC_ADD_FPC1020_NODE)
#define FPC_PROC_FILE "fpc1020_info"
static struct proc_dir_entry *g_fpc_proc = NULL;
#endif
/* -------------------------------------------------------------------- */
...
#endif
static struct spi_device_id spi_id_table = { "fpc1020", 0 };
...
//	int ret;
	...
	switch (cmd) {
	case 1:
		fpc_pin_default = pinctrl_lookup_state(fpc_pinctrl, "state_eint_as_int");
		if (IS_ERR(fpc_pin_default)) {
			//ret = PTR_RRR(fpc_pin_default);
			printk("fpc cannot find pinctrl default\n");
			return -1;
		}
		pinctrl_select_state(fpc_pinctrl, fpc_pin_default);
		printk("shaohui set eint mode\n");
		break;
	case 0:
...
#if 1
	struct device_node *node;
	struct platform_device *fpc_dev=NULL;
	//	struct pinctrl_dev *pinctrl_d;
//	int ret;

	node = of_find_compatible_node(NULL, NULL, "mediatek,fpc1145");

	fpc_dev = of_find_device_by_node(node);
// 	pinctrl_d= (struct pinctrl_dev *)get_pinctrl_dev_from_of_node(node);
//	pinctrl_d = (struct pinctrl_dev *)of_pinctrl_get(node);
	if (node) {
		fpc_pinctrl = devm_pinctrl_get(&(fpc_dev->dev));
		if (IS_ERR(fpc_pinctrl)) {
		//	ret = PTR_RRR(pinctrl);
			printk("fpc cannot find pinctrl\n");
		}
		return -1;
	}
...
#if defined(FPC_ADD_FPC1020_NODE)
static int fpc_info_read_proc(struct file *file, char *buffer, size_t count,  loff_t *ppos)
{
	char *page = NULL;
	char *ptr = NULL;
	...
	else if((fpc_id == 0x111) || (fpc_id == 0x121))
	{
		manu_string = "1";
	}
	else
...
#else
#if 0
	#ifndef CONFIG_PINCTRL_MT6753
	gpio_set_value(0x80000000|256, 1);
	#else
	error = gpio_request(7, NULL);
	if (error) {
		dev_err(&spi->dev, "Could not request GPIO#256.\n");
		/*return -EIO;*/
	}
	error = gpio_direction_output(7, 1);
	if (error) {
...
#if 0
	error = gpio_request(198, NULL);
	if (error) {
		dev_err(&spi->dev, "Could not request GPIO#256.\n");
		/*return -EIO;*/
	}
	error = gpio_direction_output(198, 1);
	if (error) {
		dev_err(&spi->dev, "Could not set GPIO#256 as output.\n");
		/*return -EIO;*/
	}
...
//	error = fpc1020_irq_init(fpc1020, fpc1020_pdata);
	...
	#if defined(FPC_ADD_FPC1020_NODE)
		g_fpc_proc =proc_create(FPC_PROC_FILE, 0444, NULL,&fpc_info_proc_fops);
		if (g_fpc_proc == NULL) {
					...
					struct fpc1020_platform_data *pdata)
{
	...
	int error = 0;
		...
		fpc1020->reset_gpio =125;// pdata->reset_gpio;
#ifndef CONFIG_PINCTRL_MT6753
